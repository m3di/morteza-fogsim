...
public class ModulePlacementEdgewards extends ModulePlacement {
    ...
    protected Map<Integer, Double> currentRamOccupied;
    ...
    public ModulePlacementEdgewards(List<FogDevice> fogDevices, List<Sensor> sensors, List<Actuator> actuators, Application application, ModuleMapping moduleMapping) {
        ...
        setCurrentRamOccupied(new HashMap<Integer, Double>());
        ...
        for (FogDevice dev : getFogDevices()) {
            ...
            getCurrentRamOccupied().put(dev.getId(), 0.0);
     ...
     *
     * @param placedModules Modules that have already been placed in current
     *                      path
    ...
    private void placeModulesInPath(List<Integer> path) {
        System.out.println("filling path " + printPathName(path));
        ...
        /**
         * Periodic edges have a fixed periodicity of tuples, so setting the
         * tuple rate beforehand
         */
        ...
        for (Integer deviceId : path) {
            ...
            System.out.println("now checking " + device.getName());
...
//            System.out.println("--- assosiated sensores: " + sensorsAssociated);
//            System.out.println("--- assosiated actuators: " + actuatorsAssociated);
//            System.out.println("--- placed modules: " + placedModules);
            ...
            while (modulesToPlace.size() > 0) { // Loop runs until all modules in modulesToPlace are deployed in the path
                ...
                System.out.println("try to place " + moduleName);
...
//                System.out.println("--- upstream device id: " + upsteamDeviceId);

                if (upsteamDeviceId > 0) {
                    System.out.println("module is exists on " + getFogDeviceById(upsteamDeviceId).getName());
                    ...
                    if (upsteamDeviceId == deviceId) {
                        ...
                        if (totalCpuLoad + getCurrentCpuLoad().get(deviceId) > device.getHost().getTotalMips()) {
                            System.out.println("not possible [need " + totalCpuLoad + "mips - has " + (device.getHost().getTotalMips() - getCurrentCpuLoad().get(deviceId)) + "mips]");
...
//                            getCurrentRamOccupied().put(deviceId, getCurrentRamOccupied().get(deviceId) + getApplication().getModuleByName(moduleName).getRam());
                    ...
                    System.out.println("module is not exists upwards");
                    ...
                    if ((getCurrentRamOccupied().get(deviceId) + getApplication().getModuleByName(moduleName).getRam() > device.getHost().getRam())
                            || totalCpuLoad + getCurrentCpuLoad().get(deviceId) > device.getHost().getTotalMips()) {
                        System.out.println("not possible [need " + getApplication().getModuleByName(moduleName).getRam() + "mb - has " + (device.getHost().getRam() - getCurrentRamOccupied().get(deviceId)) + "mb] [need " + totalCpuLoad + "mips - has " + (device.getHost().getTotalMips() - getCurrentCpuLoad().get(deviceId)) + "mips]");
                        ...
                        getCurrentRamOccupied().put(deviceId, getApplication().getModuleByName(moduleName).getRam() + getCurrentRamOccupied().get(deviceId));
    ...
    protected String printPathName(List<Integer> path) {
        String acc = "[";

        for (Integer id : path) {
            acc += getFogDeviceById(id).getName() + ", ";
        }
    ...
    private List<String> shiftModuleNorth(String moduleName, double cpuLoad, Integer deviceId, List<String> operatorsToPlace) {
        ...
        for (String module : modulesToShift) {
            ...
            totalRamOccuopied += getApplication().getModuleByName(module).getRam();
        ...
        getCurrentRamOccupied().put(deviceId, getCurrentRamOccupied().get(deviceId) - totalRamOccuopied); // change info of current ram occupied on device
        ...
        while (true) { // Loop iterates over all devices in path upstream from current device. Tries to place modules (to be shifted northwards) on each of them.
            ...
            if (getCurrentRamOccupied().get(id) + totalRamOccuopied > fogDevice.getHost().getRam()
                    || getCurrentCpuLoad().get(id) + totalCpuLoad > fogDevice.getHost().getTotalMips()) {
                ...
                for (String module : _modulesToShift) {
                    if (!modulesToShift.contains(module)) {
                        ...
                        ramOccupiedShifted += getApplication().getModuleByName(module).getRam();
                        totalRamOccuopied += getApplication().getModuleByName(module).getRam();
                ...
                getCurrentRamOccupied().put(id, getCurrentRamOccupied().get(id) - ramOccupiedShifted); // CPU load on device id gets reduced due to modules shifting northwards
                ...
                for (String module : loadMap.keySet()) {
                    ...
                    totalRam += getApplication().getModuleByName(module).getRam();
                ...
                getCurrentRamOccupied().put(id, totalRam);
    ...
    /**
     * Get all modules that need to be shifted northwards along with
     * <b>module</b>. Typically, these other modules are those that are hosted
     * on device with ID <b>deviceId</b> and lie upstream of <b>module</b> in
     * application model.
     *
    ...
    /**
     * Get all modules that need to be shifted northwards along with
     * <b>modules</b>. Typically, these other modules are those that are hosted
     * on device with ID <b>deviceId</b> and lie upstream of modules in
     * <b>modules</b> in application model.
     *
    ...
    protected List<List<Integer>> getPaths(final int fogDeviceId) {
        ...
        for (int childId : device.getChildrenIds()) {
            ...
            for (List<Integer> childPath : childPaths) {
                ...
                addNeighbours(device, childPath);
            }
    ...
    protected void addNeighbours(FogDevice device, List<Integer> path) {
        for (Integer nID : device.getNeighboursLatencyMap().keySet()) {
            if (!path.contains(nID)) {
                path.add(nID);
                addNeighbours((FogDevice) CloudSim.getEntity(nID), path);
            }
        }
    }
    ...
    public Map<Integer, Double> getCurrentRamOccupied() {
        return currentRamOccupied;
    }

    public void setCurrentRamOccupied(Map<Integer, Double> currentRamOccupied) {
        this.currentRamOccupied = currentRamOccupied;
    }
